...
def get_logbook_data():
    """
    Download logbook data, unless already downloaded
    """
    download_if_needed('http://www.knmi.nl/cliwoc/download/CLIWOC15.zip',
                       'logbook_data.zip')


def extract_logbook_data():
    """
    Fetch pronto data (if needed) and extract trips as dataframe
    """
    get_logbook_data()
    zf = zipfile.ZipFile('logbook_data.zip')
